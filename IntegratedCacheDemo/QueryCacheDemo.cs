            ...
            try
            {
                ...
                benchmarks = new List<Benchmark>
                {
                    new Benchmark
                    {
                        ...
                        testDescription = $"Perform writes to a Cosmos DB account with a query cache",
                    ...
                    new Benchmark
                 {
                        benchmarkType = BenchmarkType.CustomQuery,
                        testName = "Custom queries",
                        testDescription = $"Perform queries on a Cosmos DB account with a query cache",
                        accountEndpoint = configuration["dedicatedGatewayAccountEndpoint"],
                        accountKey = configuration["dedicatedGatewayAccountKey"],
                        databaseId = configuration["databaseId"],
                        containerId = configuration["containerId"],
        ...
        public async Task RunBenchmarks()
        {
            try
            {
                ...
                foreach (Benchmark benchmark in benchmarks)
                {
                    ...
                    else if (benchmark.benchmarkType == BenchmarkType.CustomQuery)
                        await Benchmark.CustomQueryBenchmark(benchmark);
                    else
